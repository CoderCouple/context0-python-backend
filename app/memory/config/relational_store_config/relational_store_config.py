...
class RelationalStoreConfig(BaseModel):
    """Dynamic relational store configuration supporting multiple providers"""

    provider: str = Field(default="postgresql", description="Relational store provider")
    ...
    def validate_and_create_config(self) -> "RelationalStoreConfig":
        ...
        if provider not in self._provider_configs:
            raise ValueError(
                f"Unsupported relational store provider: {provider}. "
        ...
        try:
            ...
            module = importlib.import_module(
                f"app.memory.config.relational_store_config.{provider}_config"
            )
