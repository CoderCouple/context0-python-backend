...
class WorkingMemoryHandler(BaseMemoryHandler):
    """Handler for working memory (short-term, session-specific)"""

    def __init__(self):
        super().__init__()
        self.memory_type = MemoryType.WORKING_MEMORY
    ...
    async def process(
        ...
        # Working memory has TTL and session scope
        embedding, model_name = await self.extract_embedding(record.raw_text)

        enhanced_metadata = {
            **record.metadata,
            "ttl_hours": 24,  # Working memory expires after 24 hours
            "session_bound": True,
        }
        ...
        return MemoryEntry(
            ...
            scope=f"session:{record.session_id}",
            ...
            tags=record.tags + ["working", "temporary"],
